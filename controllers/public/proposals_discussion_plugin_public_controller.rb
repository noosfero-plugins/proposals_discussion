class ProposalsDiscussionPluginPublicController < ApplicationController
  ...
  def load_proposals
    ...
    unless proposals.empty?
      render :partial => 'content_viewer/proposals_list_content', :locals => {:proposals => proposals, :holder => holder, :next_page => page+1}
    else
      render :text => ''
    end
  end
   ...
   def set_rand_cookie
     ...
     cookies[:_noosfero_proposals_discussion_rand_seed] = {value: rand, expires: Time.now + 900}
   end
